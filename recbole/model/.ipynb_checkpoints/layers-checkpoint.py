...
class kAttentionPooling(nn.Module):
    def __init__(self, seq_len, hidden_size, k_heads=5):
        ...
        #self.klinear = nn.Linear(seq_len, self.k_heads)
        self.theta_k = nn.Parameter(torch.randn([hidden_size, k_heads]))
        
    def forward(self, input_tensor): # [B, L, d] -> [B, k, d]
        
        attention_matrix = torch.matmul(input_tensor, self.theta_k) #[B, L, k]
        #attention_matrix = attention_matrix / math.sqrt(self.k_heads) #[B, L, k]
        attention_matrix = nn.Softmax(dim=-2)(attention_matrix)
        pooling_result = torch.einsum('nij, nik -> nkj', input_tensor, attention_matrix) # #[B, k, d]

        return pooling_result
'''
class MultiHeadAttentionWithPooling(nn.Module): # internal
    """
    ...
    """
    def __init__(self, n_heads, k_heads, hidden_size, seq_len, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        ...
        self.attpooling_pos_k = kAttentionPooling(seq_len, hidden_size, k_heads)
    ...
    def forward(self, input_tensor, pos_emb):
        ...
        # 需要考虑原始的MultiHead和pooling里的Khead的关系
        ...
        pos_key_layer = self.transpose_for_scores(self.attpooling_pos_k(self.pos_k_linear(pos_emb)))
        abs_pos_bias = torch.matmul(pos_query_layer, pos_key_layer.transpose(-1, -2))

        attention_scores = attention_scores + abs_pos_bias
...
class MultiHeadAttentionWithPooling(nn.Module): # external
    ...
    def forward(self, input_tensor, pos_emb):
        ...
        # MAIN CHANGE: add external position
        # position relation: only position embedding
        ...
        #abs_pos_bias = abs_pos_bias + attention_mask
        abs_pos_bias = nn.Softmax(dim=-2)(abs_pos_bias)
        
        #abs_pos_bias = abs_pos_bias.expand_as(value_layer_pos)
        context_layer_pos = torch.matmul(abs_pos_bias, value_layer_pos)

        # relation of items + relation of positions
...
class TransformerLayer(nn.Module):
    ...
    """
    def __init__(self, n_heads, k_heads, hidden_size, seq_len, intermediate_size,
                 hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps):
        super(TransformerLayer, self).__init__()
        self.multi_head_attention = MultiHeadAttentionWithPooling(n_heads, k_heads, hidden_size,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        layer = TransformerLayer(n_heads, k_heads, hidden_size, seq_len, inner_size,
                                 hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps)
